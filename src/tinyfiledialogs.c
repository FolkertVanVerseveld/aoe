...
#define tfdquery(txt,code,type) if (aTitle && !strcmp(aTitle, TINYFD_QUERY)) {\
		strcpy(tinyfd_response, txt); \
		return (type)code;\
	}
...
)
{
	...
	if (aSource) {
		...
		if (!lTmp)
			lTmp = strrchr(aSource, '\\');
		strcpy(aoDestination, lTmp ? lTmp + 1 : aSource);
	} else
		*aoDestination = '\0';
	return aoDestination;
}
...
{
	if (!aFileNameWithoutPath || !*aFileNameWithoutPath || strpbrk(aFileNameWithoutPath, "\\/:*?\"<>|"))
		return 0;
	return 1;
}
...
{
	FILE * lIn;
	if (!aFilePathAndName || !*aFilePathAndName)
		return 0;
...
static wchar_t *utf8to16(char const *const aUtf8string)
{
	...
	lUtf16string = malloc(lSize * sizeof(wchar_t));
...
static void runSilentA(char const *const aString)
{
	...
	lArgs = malloc(MAX_PATH_OR_CMD + lStringLen);

	pEnvCMD = getenv("COMSPEC");

	strcpy(lArgs, pEnvCMD ? pEnvCmd : pDefaultCMD);
...
static void runSilentW(wchar_t const *const aString)
{
	...
	lArgs = malloc((MAX_PATH_OR_CMD + lStringLen) * sizeof(wchar_t));
...
)
{
	...
	lDialogString = malloc(3 * MAX_PATH_OR_CMD + lTitleLen + lMessageLen);
    ...
    int const aDefaultButton) /* 0 for cancel/no, 1 for ok/yes */
{
	...
	if (((aDialogType && strcmp("okcancel", aDialogType) && strcmp("yesno", aDialogType))) || (lBoxReturnValue == IDOK) || (lBoxReturnValue == IDYES))
		return 1;
	else
		return 0;
}
...
)
{
	...
	if (aNumOfFilterPatterns > 0) {
		...
		strcat(lFilterPatterns, "\n");
		if (!(aSingleFilterDescription && *aSingleFilterDescription)) {
...
static int dialogPresent()
{
	...
	if (lDialogPresent < 0) {
		...
		while (fgets(lBuff, sizeof lBuff, lIn))
			;
		_pclose(lIn);
	...
	char const *const aDefaultInput) /* "", if NULL it's a passwordBox */
{
	...
	if (aMessage && *aMessage)
		strcat(lDialogString, aMessage);
	strcat(lDialogString, "\" 10 60 ");
	if (aDefaultInput && *aDefaultInput) {
		strcat(lDialogString, "\"");
		strcat(lDialogString, aDefaultInput);
		strcat(lDialogString, "\" ");
	}
	...
	while (fgets(aoBuff, MAX_PATH_OR_CMD, lIn))
		;
	fclose(lIn);
	remove(lDialogFile);
...
)
{
	...
	strcat(lDialogString, "--backtitle \"");
	strcat(lDialogString, "tab -> focus | spacebar -> select | / -> populate | enter -> ok input line");
	strcat(lDialogString, "\" ");
...
)
{
	...
	if ((!tinyfd_forceConsole || !(GetConsoleWindow() || dialogPresent())) && (!getenv("SSH_CLIENT") || getenv("DISPLAY"))) {
		tfdquery("windows", 1, char*);
		if (tinyfd_winUtf8)
			p = openFileDialogWinGui8(lBuff, aTitle, aDefaultPathAndFile, aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription, aAllowMultipleSelects);
		else
			p = openFileDialogWinGuiA(lBuff, aTitle, aDefaultPathAndFile, aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription, aAllowMultipleSelects);
	} else
#endif /* TINYFD_NOLIB */
	if (dialogPresent()) {
		tfdquery("dialog", 0, char*);
...
static int detectPresence(char const *const aExecutable)
{
	...
	if (fgets(lBuff, sizeof lBuff, lIn) && !strchr(lBuff, ':')) {
		/* present */
		pclose(lIn);
		return 1;
	} else {
...
static char const *terminalName()
{
	...
	if (lTerminalName[0] == '*') {
		...
		else if (strcpy(lTerminalName, "terminator") && detectPresence(lTerminalName)) /*good*/
			strcat(lTerminalName, " -x bash -c ");
		else if (strcpy(lTerminalName, "lxterminal") && detectPresence(lTerminalName)) /*good*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "mate-terminal") && detectPresence(lTerminalName)) /*good*/
			strcat(lTerminalName, " -x bash -c ");
		else if (strcpy(lTerminalName, "konsole") && detectPresence(lTerminalName))
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "rxvt") && detectPresence(lTerminalName)) /*good*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "urxvt") && detectPresence(lTerminalName)) /*good*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "mrxvt") && detectPresence(lTerminalName)) /*good*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "evilvte") && detectPresence(lTerminalName)) /*good*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "termit") && detectPresence(lTerminalName)) /*good*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "kterm") && detectPresence(lTerminalName)) /*good*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "roxterm") && detectPresence(lTerminalName)) /*good*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "xterm") && detectPresence(lTerminalName)) /*good small*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "lxterm") && detectPresence(lTerminalName)) /*good small*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "xvt") && detectPresence(lTerminalName)) /*good B&W*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "pterm") && detectPresence(lTerminalName)) /*good only letters*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "x-terminal-emulator") && detectPresence(lTerminalName)) /*alias*/
			strcat(lTerminalName, " -e bash -c ");
		else if (strcpy(lTerminalName, "$TERM") && detectPresence(lTerminalName)) /*alias*/
			strcat(lTerminalName, " -x bash -c ");
		else
			strcpy(lTerminalName, "");
...
static char const *dialogName()
{
	...
	if (lDialogName[0] == '*')
		if ((isDarwin() && strcpy(lDialogName, "/opt/local/bin/dialog") && detectPresence(lDialogName)) || (strcpy(lDialogName, "dialog") && detectPresence(lDialogName))) {}
...
static int zenity3Present()
{
	...
	if (lZenity3Present < 0)
	{
		if (!zenityPresent())
			...
			if (fgets(lBuff, sizeof lBuff, lIn) && (atoi(lBuff) >= 3
...
)
{
	...
	lMessageLen =  aMessage ? strlen(aMessage) : 0;
	if (!aTitle || strcmp(aTitle, TINYFD_QUERY))
		lDialogString = malloc(MAX_PATH_OR_CMD + lTitleLen + lMessageLen);

	if (osascriptPresent()) {
		tfdquery("applescript", 1, int);
		...
		if (zenityPresent()) {
			tfdquery("zenity", 1, int);
			strcpy(lDialogString, "zenity --");
		} else {
			tfdquery("matedialog", 1, int);
	...
	} else if (!xdialogPresent() && !gdialogPresent() && notifysendPresent() && strcmp("okcancel", aDialogType) && strcmp("yesno", aDialogType)) {
		tfdquery("notify", 1, int);
		...
		if (aMessage && *aMessage)
			strcat(lDialogString, aMessage);
		strcat(lDialogString, "\"");
	} else if (!xdialogPresent() && !gdialogPresent() && xmessagePresent() && strcmp("okcancel", aDialogType) && strcmp("yesno", aDialogType)) {
		tfdquery("xmessage", 1, int);
...
)
{
	...
	if (osascriptPresent()) {
		...
		if (zenityPresent()) {
			tfdquery("zenity", 1, char*);
      strcpy(lDialogString,  "szAnswer=$(zenity --entry");
		} else {
			tfdquery("matedialog", 1, char*);
		...
		if (xdialogPresent()) {
			tfdquery("xdialog", 1, char*);
			lWasGraphicDialog = 1;
			strcpy(lDialogString, "(Xdialog ");
		} else if (gdialogPresent()) {
			tfdquery("gdialog", 1, char*);
			lWasGraphicDialog = 1;
			lWasGdialog = 1;
			strcpy(lDialogString, "(gdialog ");
		} else if (dialogName()) {
		...
		if (lWasGraphicDialog)
			strcat(
				lDialogString, ") 2>/tmp/tinyfd.txt;"
			...
			strcat(
				lDialogString, ">/dev/tty) 2>/tmp/tinyfd.txt;"
				"if [ $? = 0 ];then tinyfdBool=1;else tinyfdBool=0;fi;"
				"tinyfdRes=$(cat /tmp/tinyfd.txt);"
				"rm /tmp/tinyfd.txt;echo $tinyfdBool$tinyfdRes"
			);
		...
		if (aTitle && *aTitle && !tinyfd_forceConsole) {
			strcat(lDialogString, "echo \"");
			strcat(lDialogString, aTitle);
			strcat(lDialogString, "\";echo;");
		}
    ...
    const char *aSingleFilterDescription) /* NULL or "image files" */
{
	...
	if (osascriptPresent()) {
		...
		/* test because poor osx behaviour */
		if (aNumOfFilterPatterns > 1 || (aNumOfFilterPatterns == 1 && aFilterPatterns[0][strlen(aFilterPatterns[0]) - 1] != '*')) {
